...
export default function InventoryPage() {
  ...
  const fetchInventoryData = async () => {
    try {
      ...
      const [stockData, issuanceData, movementData, operatorData] = await Promise.all([
      ...
      console.log("[v0] Stock items fetched:", stockData.length)
      console.log("[v0] Issuances fetched:", issuanceData.length)
      console.log("[v0] Stock movements fetched:", movementData.length)
  ...
  const handleDeleteItem = async (item: StockItem) => {
    try {
      ...
      await inventoryApi.deleteStockItem(item.id, "Admin")
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-500 mt-2">In {categories.length} categories</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">Recent Issues</CardTitle>
              <TrendingDown className="h-5 w-5 text-orange-600" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-gray-900">{issuances.length}</div>
                    ...
                    <TableBody>
                      {filteredItems.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            <div>
                              <div className={`font-medium ${item.quantity < 10 ? "text-red-600" : "text-gray-900"}`}>
                                {item.quantity} {item.unitType || "pcs"}
                              </div>
                              {item.quantity < 10 && <div className="text-xs text-red-500">Low Stock</div>}
                            </div>
                          </TableCell>
                          <TableCell>
                            <div>
                              ...
                              {item.sellingPrice && item.sellingPrice !== item.unitPrice && (
                                <div className="text-sm text-gray-500">Sell: {formatCurrency(item.sellingPrice)}</div>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>
          ...
          <TabsContent value="issue" className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">Track inventory distribution</p>
              </div>
      ...
      <TableRow key={issue.IssueID}>
        ...
        <TableCell>
          <div>
            ...
            <div className="text-sm text-gray-500">{issue.OperatorID}</div>
            {issue.operatorEmail && (
              <div className="text-xs text-gray-400">{issue.operatorEmail}</div>
            )}
          </div>
        </TableCell>
        <TableCell>
          <div className="space-y-1">
            {issue.Items && Array.isArray(issue.Items) && issue.Items.map((item, index) => (
                    ...
                    <TableBody>
                      {stockMovements.map((log) => (
                        <TableRow key={log.id}>
                          ...
                          <TableCell className="font-medium">{log.itemId}</TableCell>
                          <TableCell>
                            <Badge
                              variant="outline"
                              className={
                          ...
                          <TableCell>
                            <span className="text-gray-600">{log.quantity}</span>
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center">
                          ...
                          <TableCell className="text-sm text-gray-600">{log.to && `To: ${log.to}`}</TableCell>
                        </TableRow>
                      ))}
...
function AddItemForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await inventoryApi.addStockItem({
        ...formData,
        role: "Admin",
      })
...
function EditItemForm({ item, onClose, onSuccess }: { item: StockItem; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await inventoryApi.updateStockItem(item.id, {
...
function IssueInventoryForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await inventoryApi.issueStockToOperator({
