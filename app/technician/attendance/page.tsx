...
export default function AttendancePage() {
  ...
  const handleCheckIn = () => {
    ...
    const updatedData = attendanceData.map((record) =>
      record.date === today ? { ...record, status: "present", checkIn: now.toTimeString().split(" ")[0] } : record,
    )

    // If today's record doesn't exist, add it
    if (!attendanceData.find((record) => record.date === today)) {
      updatedData.push({
        date: today,
        status: "present",
  ...
  const handleCheckOut = () => {
    ...
    setAttendanceData(
      attendanceData.map((record) =>
        record.date === today ? { ...record, checkOut: now.toTimeString().split(" ")[0] } : record,
      ),
    )
    setHasChanges(true)
  }
  ...
  const handleApplyChanges = () => {
    console.log("Applying attendance changes:", attendanceData)
    setHasChanges(false)
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="flex items-center space-x-3 p-4 bg-white/60 rounded-lg backdrop-blur-sm">
              ...
              <div>
                ...
                <p className="font-bold text-lg text-gray-900">{currentAttendance.checkInTime}</p>
              </div>
            </div>
