...
export default function TechnicianManagement() {
  ...
  const fetchTechniciansData = async () => {
    try {
      setLoading(true)
      const [techniciansData, tasksData] = await Promise.all([
        apiClient.getAllTechnicians(),
        apiClient.getAllComplaints(), // Using complaints as tasks for now
      ])
      ...
      toast({
        title: "Data Loaded",
        description: "Technician data loaded successfully!",
      })
    } catch (error) {
      console.error("[v0] Error fetching technicians data:", error)
      toast({
        title: "Error Loading Data",
        description: "Failed to load technician data. Please try again.",
        variant: "destructive",
      })
      setTechnicians([])
      setTasks([])
  ...
  const handleAddTechnician = async () => {
    try {
      ...
      await apiClient.addTechnician(technicianData)
      ...
      console.error("[v0] Error adding technician:", error)
      toast({
        title: "Add Failed",
              ...
              <CardContent>
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className="text-sm text-green-600 font-medium">
                    {Math.round((technicianStats.completedTasksToday / technicianStats.totalTasksToday) * 100)}%
                    completed
                  </span>
                </div>
              </CardContent>
                          ...
                          <TableRow key={tech.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <TableCell>
                              <div className="flex items-center space-x-2">
                                ...
                                <Select
                                  value={tech.status}
                                  onValueChange={(value) => handleStatusUpdate(tech.id, value)}
                                >
                      ...
                      <TableBody>
                        {technicians.map((tech) => (
                          <TableRow key={tech.id}>
                            ...
                            <TableCell>
                              {tech.checkInTime && tech.checkOutTime
                                ? "8h 30m"
                                : tech.checkInTime
                                  ? "In Progress"
                                  : "-"}
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div>
                          ...
                          {Math.round((selectedTechnician.tasksCompleted / selectedTechnician.tasksAssigned) * 100) ||
                            0}
                          %
                        </div>
                        <div>
